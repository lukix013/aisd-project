...
std::vector<int> zmodyfikujDane(const std::string& filePath, int mode) {
...
void printEdgeList(const std::vector<int>& edges, int N) {
    if (edges.empty()) {
    ...
    int source = N;
    int sink = N + 1;
    std::cout << "Graph Edge List (from, to, capacity):" << std::endl;
    for (size_t i = 0; i < edges.size(); i += 3) {
        int u = edges[i];
        int v = edges[i + 1];
        int cap = edges[i + 2];

        std::cout << "  " << u_str << " -> " << v_str << " (cap: " << cap << ")" << std::endl;
    }
}
/*
